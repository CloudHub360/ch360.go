...
func CommandFor(runParams *config.RunParams, apiClient *ch360.ApiClient) (Command, error) {
	...
	if runParams.Verb() == config.Classify {
		...
		switch runParams.Verb() {
		case config.Create:
			return NewCreateExtractorWithArgs(runParams, apiClient.Extractors, out)
		case config.Upload:
			return NewUploadExtractorFromArgs(runParams, apiClient.Extractors, out)
		case config.Delete:
		...
		return NewCreateExtractorTemplateWithArgs(runParams, apiClient.Modules)
